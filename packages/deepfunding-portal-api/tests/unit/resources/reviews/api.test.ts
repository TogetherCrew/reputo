...
describe('Review API', () => {
  describe('fetchReviews', () => {
    ...
    it('should include reviewerId filter when provided', async () => {
      const mockResponse: ReviewApiResponse = {
        reviews: [],
        pagination: {
          current_page: 1,
      ...
      const results: unknown[] = [];
      for await (const page of fetchReviews(client, { reviewerId: 10 })) {
        results.push(...page.data);
      }

      expect(client.mockGet).toHaveBeenCalledWith('/reviews', { page: 1, limit: 500, reviewer_id: 10 });
    });
    ...
    it('should include proposalId filter when provided', async () => {
      ...
      const results: unknown[] = [];
      for await (const page of fetchReviews(client, { proposalId: 20 })) {
        results.push(...page.data);
      }

      expect(client.mockGet).toHaveBeenCalledWith('/reviews', { page: 1, limit: 500, proposal_id: 20 });
    });
    ...
    it('should include type filter when provided', async () => {
      ...
      const results: unknown[] = [];
      for await (const page of fetchReviews(client, { type: 'expert' })) {
        results.push(...page.data);
      }

      expect(client.mockGet).toHaveBeenCalledWith('/reviews', { page: 1, limit: 500, type: 'expert' });
    });
  });
});
