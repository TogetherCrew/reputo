[**@reputo/database v0.0.0**](README.md)

***

# @reputo/database v0.0.0

## Interfaces

- [PaginateOptions](interfaces/PaginateOptions.md)
- [PaginateResult](interfaces/PaginateResult.md)
- [AlgorithmPreset](interfaces/AlgorithmPreset.md)
- [AlgorithmPresetModel](interfaces/AlgorithmPresetModel.md)
- [AlgorithmPresetFrozen](interfaces/AlgorithmPresetFrozen.md)
- [SnapshotOutputs](interfaces/SnapshotOutputs.md)
- [SnapshotError](interfaces/SnapshotError.md)
- [Snapshot](interfaces/Snapshot.md)
- [SnapshotModel](interfaces/SnapshotModel.md)

## Type Aliases

- [ModelName](type-aliases/ModelName.md)
- [SnapshotStatus](type-aliases/SnapshotStatus.md)
- [AlgorithmPresetDoc](type-aliases/AlgorithmPresetDoc.md)
- [AlgorithmPresetWithId](type-aliases/AlgorithmPresetWithId.md)
- [SnapshotDoc](type-aliases/SnapshotDoc.md)
- [SnapshotWithId](type-aliases/SnapshotWithId.md)

## Variables

- [AlgorithmPresetModelValue](variables/AlgorithmPresetModelValue.md)
- [SnapshotModelValue](variables/SnapshotModelValue.md)
- [AlgorithmPresetSchema](variables/AlgorithmPresetSchema.md)
- [SnapshotSchema](variables/SnapshotSchema.md)
- [MODEL\_NAMES](variables/MODEL_NAMES.md)
- [SNAPSHOT\_STATUS](variables/SNAPSHOT_STATUS.md)

## Functions

- [connect](functions/connect.md)
- [disconnect](functions/disconnect.md)
- [paginate](functions/paginate.md)
