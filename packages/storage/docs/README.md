...
console.log(upload.key) // 'uploads/{uuid}/votes.csv'
...
```typescript
const download = await storage.presignGet('uploads/{uuid}/votes.csv')
...
await storage.putObject('uploads/{uuid}/results.csv', csvData, 'text/csv')
```
...
export async function voting_engagement(
    ...
    // Write output
    const outputKey = `snapshots/${payload.snapshotId}/voting_engagement.csv`
...
#### `generateKey(type: 'upload' | 'snapshot', id: string, filename: string): string`

Generates an S3 storage key. 

- For uploads: `uploads/{uuid}/{filename}` (UUID is generated by the caller)
- For snapshots: `snapshots/{snapshotId}/{filename}`

The filename should include the extension if needed (e.g., `'votes.csv'`).
...
// Generate upload key
const uploadKey = generateKey('upload', randomUUID(), 'votes.csv')
// Returns: 'uploads/{uuid}/votes.csv'

// Generate snapshot key
const snapshotKey = generateKey('snapshot', 'abc123', 'voting_engagement.csv')
