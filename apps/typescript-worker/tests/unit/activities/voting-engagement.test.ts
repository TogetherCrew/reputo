...
voter3,proposal1,10`
        ...
        expect(mockStorage.getObject).toHaveBeenCalledWith(
            'snapshots/123/inputs/votes.csv'
        )
        ...
        const outputCsv = vi.mocked(mockStorage.putObject).mock
            .calls[0][1] as string
    ...
    it('should throw an error if votes input is missing', async () => {
        ...
        await expect(voting_engagement(payload)).rejects.toThrow(
            'Missing input "votes"'
        )
    })

    it('should handle storage errors gracefully', async () => {
        vi.mocked(mockStorage.getObject).mockRejectedValue(
