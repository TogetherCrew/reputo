...
const api = axios.create({
  baseURL: process.env.NEXT_PUBLIC_API_URL || 'https://api.pr-8-6-algorithmpreset-ip-3-87-156-226.my.preview.run/api/v1',
  headers: {
...
export const algorithmPresetsApi = {
  ...
  getAll: async (params?: AlgorithmPresetQueryParams): Promise<PaginatedAlgorithmPresetResponseDto> => {
  ...
  create: async (data: CreateAlgorithmPresetDto): Promise<AlgorithmPresetResponseDto> => {
    const response = await api.post('/algorithm-presets', data);
    return response.data;
  },

  // Update an existing algorithm preset
  update: async (id: string, data: UpdateAlgorithmPresetDto): Promise<AlgorithmPresetResponseDto> => {
...
export const snapshotsApi = {
  ...
  getAll: async (params?: SnapshotQueryParams): Promise<PaginatedSnapshotResponseDto> => {
