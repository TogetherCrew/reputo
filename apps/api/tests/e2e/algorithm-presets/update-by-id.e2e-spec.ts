...
describe('PATCH /api/v1/algorithm-presets/:id', () => {
  ...
  it('should return 404 when preset does not exist', async () => {
    ...
    await api(app, authCookie).patch(`/algorithm-presets/${fakeId}`).send({ name: 'Test' }).expect(404);
  });

  it('should reject name shorter than 3 chars (400)', async () => {
    const preset = await insertAlgorithmPreset(algorithmPresetModel);

    await api(app, authCookie).patch(`/algorithm-presets/${preset._id}`).send({ name: 'ab' }).expect(400);
  });
  ...
  it('should reject description shorter than 10 chars (400)', async () => {
    ...
    await api(app, authCookie).patch(`/algorithm-presets/${preset._id}`).send({ description: 'short' }).expect(400);
  });
  ...
  it('should reject attempts to update key (400)', async () => {
    ...
    const res = await api(app, authCookie).patch(`/algorithm-presets/${preset._id}`).send({ key: 'new_key' });
  ...
  it('should reject attempts to update version (400)', async () => {
    ...
    const res = await api(app, authCookie).patch(`/algorithm-presets/${preset._id}`).send({ version: '2.0.0' });
