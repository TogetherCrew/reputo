...
describe('AlgorithmPresetService', () => {
    ...
    beforeEach(() => {
        ...
        mockConfigService = {
            get: vi.fn((key: string) => {
                if (key === 'storage.maxSizeBytes')
                    return defaultStorageConfig.maxSizeBytes
                if (key === 'storage.contentTypeAllowlist')
                    return defaultStorageConfig.contentTypeAllowlist
                return undefined
            }),
    ...
    describe('create', () => {
        ...
        it('should validate storage metadata and CSV content for csv inputs', async () => {
            ...
            expect(mockStorageService.getObjectMetadata).toHaveBeenCalledWith(
                'uploads/test.csv'
            )
            expect(mockStorageService.getObject).toHaveBeenCalledWith(
                'uploads/test.csv'
            )
            expect(validateCSVContent).toHaveBeenCalledWith(
                validCsvBuffer,
    ...
    describe('validateStorageInputs - metadata validation', () => {
        it('should throw CSVValidationException when content type is not allowed', async () => {
            ...
            mockStorageService.getObjectMetadata = vi
                .fn()
                .mockResolvedValue(invalidMetadata)
            ...
            try {
                ...
                const response = (
                    error as CSVValidationException
                ).getResponse() as any
        ...
        it('should throw CSVValidationException when file exceeds API config maxSizeBytes', async () => {
            ...
            try {
                ...
                expect(response.errors[0].errors).toContainEqual(
                    expect.stringContaining('exceeds API limit')
                )
            }
        })
        ...
        it('should throw CSVValidationException when file exceeds algorithm definition maxBytes', async () => {
            ...
            vi.mocked(getAlgorithmDefinition).mockReturnValue(
                JSON.stringify(definitionWithMaxBytes)
            )
            ...
            try {
                ...
                expect(response.errors[0].errors).toContainEqual(
                    expect.stringContaining('exceeds algorithm limit')
                )
            }
        })
        ...
        it('should throw CSVValidationException when CSV content is invalid', async () => {
            ...
            try {
                ...
                expect(response.errors[0].errors).toContain(
                    'Missing required column: column1'
                )
            }
        })
    ...
    describe('validateStorageInputs - multiple errors collection', () => {
        it('should collect metadata and CSV content errors together', async () => {
            ...
            try {
                ...
                expect(response.errors[0].errors.length).toBeGreaterThanOrEqual(
                    2
                )
                expect(response.errors[0].errors).toContainEqual(
        ...
        it('should collect errors from multiple CSV inputs', async () => {
                ...
                inputs: [
                    {
                        ...
                        csv: {
                            ...
                            delimiter: ',',
                            columns: [
                                { key: 'col1', type: 'string', required: true },
                            ],
                        },
                    },
                    {
                        ...
                        csv: {
                            ...
                            delimiter: ',',
                            columns: [
                                { key: 'col2', type: 'string', required: true },
                            ],
                        },
                    },
                ],
            ...
            vi.mocked(getAlgorithmDefinition).mockReturnValue(
                JSON.stringify(multiInputDefinition)
            )
    ...
    describe('validateStorageInputs - skip non-CSV inputs', () => {
        it('should skip validation for non-csv input types', async () => {
            ...
            vi.mocked(getAlgorithmDefinition).mockReturnValue(
                JSON.stringify(mixedInputDefinition)
            )
            ...
            expect(mockStorageService.getObjectMetadata).toHaveBeenCalledTimes(
                1
            )
