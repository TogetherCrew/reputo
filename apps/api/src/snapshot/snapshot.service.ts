...
export class SnapshotService {
    ...
    async create(createDto: CreateSnapshotDto) {
        const algorithmPreset = await this.algorithmPresetRepository.findById(
    ...
    private async startWorkflowAsync(snapshotId: string): Promise<void> {
        try {
            ...
            this.logger.error(
                `Failed to start workflow for snapshot ${snapshotId}: ${err.message}`,
                err.stack,
                { snapshotId }
            )
    ...
    list(queryDto: ListSnapshotsQueryDto) {
        ...
        const paginateOptions = pick(queryDto, [
            'page',
            'limit',
            'sortBy',
            'populate',
        ...
        if (queryDto.key)
            presetFilters['algorithmPresetFrozen.key'] = queryDto.key
        if (queryDto.version)
            presetFilters['algorithmPresetFrozen.version'] = queryDto.version
