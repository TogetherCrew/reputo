...
import { type DeepIdDiscoveryDocument, type DeepIdTokenResponse, type DeepIdUserInfo } from '../types';

interface BuildDeepIdAuthorizationUrlParams {
...
interface ExchangeDeepIdCodeParams {
  code: string;
  codeVerifier: string;
  redirectUri: string;
}
...
async function parseJsonResponse<T>(response: Response): Promise<T> {
  ...
  try {
    ...
    throw new BadGatewayException('Deep ID returned malformed JSON.');
  }
}

@Injectable()
export class DeepIdOAuthClient {
  ...
  async buildAuthorizationUrl(params: BuildDeepIdAuthorizationUrlParams): Promise<string> {
  ...
  async exchangeCodeForTokens(params: ExchangeDeepIdCodeParams): Promise<DeepIdTokenResponse> {
    this.ensureDeepIdMode();
